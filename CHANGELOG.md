...
- `login --oauth` and `login --device` now add `utm_source` and `utm_medium` to the sign-in URL they open, so Maton can tell which agent a sign-in came from. Signing in is otherwise unchanged, and the only value sent is the agent name the CLI already reports in `X-Maton-Client-User-Agent`.
